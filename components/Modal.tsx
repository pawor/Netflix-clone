...
import { XIcon, PlusIcon, ThumbUpIcon, VolumeOffIcon } from '@heroicons/react/outline'
...
function Modal(){
    ...
    return (
        ...
        >
            <>
                <button onClick={handleClose} className="absolute modalButton right-5 top-5 !z-40 border-none h-9 w-9 bg-[#181818] hover:bg-[#181818]">
                ...
                <div className='relative pt-[56.25%]'>
                    ...
                    <div className='absolute flex items-center justify-between w-full px-10 bottom-10'>
                        ...
                        <button className='modalButton' onClick={() => setMuted(!muted)}>
                            {muted ? (<VolumeOffIcon className='w-6 h-6'/>):(<VolumeOffIcon className='w-6 h-6'/>)}
                        </button>
                    </div>
                </div>
                
                <div className='flex space-x-16 text-sm rounded-b-md bg-[#181818] px-10 py-8'>
                    <div className='space-y-6 text-lg'>
                        <div className='flex items-center space-x-2 text-sm'>
                            <p className='font-semibold text-green-400'>{movie!.vote_average * 10}% Match</p>
                            <p className='font-light'>{movie?.release_date || movie?.first_air_date}</p>
                            <div className='flex items-center justify-center h-4 px-1.5 border rounded border-white/40 text-xs'>HD</div>
                        </div>
